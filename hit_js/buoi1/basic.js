function ptb2() {
  ...
  if (a == 0) {
    if (b == 0 && c == 0) result.textContent = "Phương trình có vô số nghiệm";
    else if (b == 0 && c != 0) result.textContent = "Phương trình vô nghiệm";
    else result.textContent = "x = " + -c / b;
  } else {
    const delta = b * b - 4 * a * c;
    if (delta < 0) result.textContent = "Phương trình vô nghiệm";
    else if (delta == 0) 
...
function findSNT() {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (isPrimeNumber) primeNumbersString += i + " ";
  }
  if (primeNumbersString !== "") sntResult.textContent = primeNumbersString;
  else sntResult.textContent = -1;
}

document
  .querySelectorAll('form > button[type="reset"]')
  .forEach((rsButton, index) => {
