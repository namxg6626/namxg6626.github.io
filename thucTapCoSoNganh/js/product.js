...
class SmartphoneStuff extends Product {
    ...
    checkWarranty() {
        if (this._mfgDate == null || this._warranty == null) {
            return false;
        }
...
for (let element of listProductsRawData) {
    ...
    if (element[0] == "extension")
        myCart.add(new Extension(element[1], element[2], element[3], element[4], element[5], element[6], element[7], element[8], element[9]));
}
