...
async function refreshData() {
  ...
  if (isUserSearch) {
    let dateAtTheSearchedCity = new Date(
      Math.floor(GMTInUnixTime + current.timezone) * 1000
    );
...
async function displayForecastInDay(sourceData) {
  ...
  for (let i = 0; i < time.length; i++) {
    time[i].textContent =
      parseInt(sourceData[i].dt_txt.split(" ")[1].split(":")[0]) + "h";
    icon[i].setAttribute(
      "src",
      `https://openweathermap.org/img/wn/${sourceData[i].weather[0].icon}.png`
    );
...
async function displayCurrentWeather(sourceData) {
  ...
  document
    ...
    .setAttribute(
      "src",
      `https://openweathermap.org/img/wn/${status.icon}@2x.png`
    );
}
...
function getWeekday(dayOfWeek) {
  ...
  switch (dayOfWeek) {
    case cases[0]:
      if (lang === "vi") return "Chủ nhật";
      else return "Sunday";
    case cases[1]:
      if (lang === "vi") return "Thứ 2";
      else return "Monday";
    case cases[2]:
      if (lang === "vi") return "Thứ 3";
      else return "Tuesday";
    case cases[3]:
      if (lang === "vi") return "Thứ 4";
      else return "Wednesday";
    case cases[4]:
      if (lang === "vi") return "Thứ 5";
      else return "Thursday";
    case cases[5]:
      if (lang === "vi") return "Thứ 6";
      else return "Friday";
    case cases[6]:
      if (lang === "vi") return "Thứ 7";
